...
export const minLength = (min: number) =>
    validator(
        ...
        `value is at least ${min} characters long`
    );

export const maxLength = (max: number) =>
    validator(
        (value) => (value && value.length) <= max,
        `value is at most ${max} characters long`
    );
